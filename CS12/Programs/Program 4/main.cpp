...
//You will build a header file (IntVector.h) that declares the class interface, and a separate implementation file
//(IntVector.cpp) that implements the member functions. You will also create a test harness (main.cpp) that you use to
// test each function you are currently developing.
...
//Feel free to comment out previous tests while you are working on later member functions. However, the final test harness
//should test ALL of your member functions thoroughly. zyBooks will not grade your main function. However, we will spot check
//the assignments and take off points for any PROGRAM that does not have this test harness in the main.cpp file.
